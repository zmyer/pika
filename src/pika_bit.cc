...
void BitSetCmd::DoInitial(PikaCmdArgsType &argv, const CmdInfo *const ptr_info) {
    ...
    key_ = argv[1];
    if (!slash::string2l(argv[2].data(), argv[2].size(), (long *) &bit_offset_)) {
        res_.SetRes(CmdRes::kInvalidBitOffsetInt);
        return;
    }
    if (!slash::string2l(argv[3].data(), argv[3].size(), (long *) &on_)) {
...
void BitCountCmd::DoInitial(PikaCmdArgsType &argv, const CmdInfo *const ptr_info) {
    ...
    if (argv.size() == 4) {
        count_all_ = false;
        if (!slash::string2l(argv[2].data(), argv[2].size(), (long *) &start_offset_)) {
            res_.SetRes(CmdRes::kInvalidInt);
            return;
        }
        if (!slash::string2l(argv[3].data(), argv[3].size(), (long *) &end_offset_)) {
            res_.SetRes(CmdRes::kInvalidInt);
            return;
        }
    } else if (argv.size() == 2) {
...
void BitPosCmd::DoInitial(PikaCmdArgsType &argv, const CmdInfo *const ptr_info) {
    ...
    key_ = argv[1];
    if (!slash::string2l(argv[2].data(), argv[2].size(), (long *) &bit_val_)) {
        res_.SetRes(CmdRes::kInvalidInt);
        return;
    }
    if (bit_val_ & ~1) {
    ...
    if (argv.size() == 3) {
        ...
        pos_all_ = false;
        endoffset_set_ = false;
        if (!slash::string2l(argv[3].data(), argv[3].size(), (long *) &start_offset_)) {
            res_.SetRes(CmdRes::kInvalidInt);
            return;
        }
    } else if (argv.size() == 5) {
        ...
        if (!slash::string2l(argv[4].data(), argv[4].size(), (long *) &end_offset_)) {
            res_.SetRes(CmdRes::kInvalidInt);
            return;
        }
    } else
