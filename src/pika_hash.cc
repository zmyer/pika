...
#include "../third/nemo/include/nemo_iterator.h"
...
static void DoHSet(const std::string &key, const std::string &field, const std::string &value, CmdRes &res,
                   const std::string &exp_res) {
...
void HIncrbyCmd::DoInitial(PikaCmdArgsType &argv, const CmdInfo *const ptr_info) {
    ...
    key_ = argv[1];
    field_ = argv[2];
    if (argv[3].find(" ") != std::string::npos || !slash::string2l(argv[3].data(), argv[3].size(), (long *) &by_)) {
...
void HScanCmd::DoInitial(PikaCmdArgsType &argv, const CmdInfo *const ptr_info) {
    ...
    key_ = argv[1];
    if (!slash::string2l(argv[2].data(), argv[2].size(), (long *) &cursor_)) {
        res_.SetRes(CmdRes::kInvalidInt);
        return;
    }
    ...
    while (index < argc) {
        ...
        if (opt == "match" || opt == "count") {
            ...
            if (opt == "match") {
                pattern_ = argv[index];
            } else if (!slash::string2l(argv[index].data(), argv[index].size(), (long *) &count_)) {
                res_.SetRes(CmdRes::kInvalidInt);
                return;
            }
        } else {
...
void HScanCmd::Do() {
    ...
    for (; iter->Valid() && count_; iter->Next()) {
        if (use_pattern &&
            !slash::stringmatchlen(pattern_.data(), pattern_.size(), iter->field().data(), iter->field().size(), 0)) {
            continue;
        }
        count_--;
        cursor_++;
