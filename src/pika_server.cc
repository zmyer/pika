...
PikaServer::PikaServer() :
    ...
    //创建事件分发线程
    pika_dispatch_thread_ = new PikaDispatchThread(ips, port_, worker_num_,
                                                   pika_worker_thread_, 3000);
    //创建binlog接收线程
...
void PikaServer::Start() {
    ...
    if (ret != pink::kSuccess) {
        ...
        (ret == pink::kBindError ? ": bind port conflict" : ": other error");
    }
    ...
    if (!slaveof.empty()) {
        //解析slave节点信息
        int32_t sep = (int32_t) slaveof.find(":");
        //读取master节点的ip
...
bool PikaServer::ShouldAccessConnAsMaster(const std::string &ip) {
    slash::RWLock l(&state_protector_, false);
    DLOG(INFO) << "ShouldAccessConnAsMaster, repl_state_: " << repl_state_ << " ip: " << ip << " master_ip: " <<
    master_ip_;
//  if (repl_state_ != PIKA_REPL_NO_CONNECT && repl_state_ != PIKA_REPL_WAIT_DBSYNC && ip == master_ip_) {
    //如果master节点正处在连接slave节点阶段
...
Status PikaServer::AddBinlogSender(SlaveItem &slave, uint32_t filenum, uint64_t con_offset) {
    ...
    PikaBinlogSenderThread *sender = new PikaBinlogSenderThread(slave_ip, slave.port + 1000, readfile, filenum,
                                                                con_offset);
    ...
    if (sender->trim() == 0) {
        ...
        LOG(INFO) << "AddBinlogSender ok, tid is " << slave.sender_tid << " hd_fd: " << slave.hb_fd << " stage: " <<
        slave.stage;
        // Add sender
//    slash::MutexLock l(&slave_mutex_);
...
bool PikaServer::InitBgsaveEnv() {
    {
        ...
        char s_time[32];
        int len = (int) strftime(s_time, sizeof(s_time), "%Y%m%d%H%M%S", localtime(&bgsave_info_.start_time));
...
bool PikaServer::PurgeFiles(uint32_t to, bool manual, bool force) {
    ...
    int remain_expire_num = (int) (binlogs.size() - g_pika_conf->expire_logs_nums());
...
bool PikaServer::FlushAll() {
    ...
    int pos = (int) dbpath.find_last_of('/');
...
void PikaServer::InitKeyScan() {
    ...
    char s_time[32];
    int len = (int) strftime(s_time, sizeof(s_time), "%Y-%m-%d %H:%M:%S", localtime(&key_scan_info_.start_time));
