...
void SlaveofCmd::DoInitial(PikaCmdArgsType &argv, const CmdInfo *const ptr_info) {
    ...
    if (!slash::string2l(str_master_port.data(), str_master_port.size(), (long *) &master_port_) || master_port_ <= 0) {
    ...
    if (cur_size == 0) {
        ...
        if (!slash::string2l(str_filenum.data(), str_filenum.size(), (long *) &filenum_) || filenum_ < 0) {
            res_.SetRes(CmdRes::kInvalidInt);
            return;
        }
        std::string str_pro_offset = *it++;
        if (!slash::string2l(str_pro_offset.data(), str_pro_offset.size(), (long *) &pro_offset_) || pro_offset_ < 0) {
            res_.SetRes(CmdRes::kInvalidInt);
            return;
        }
    } else {
...
void TrysyncCmd::DoInitial(PikaCmdArgsType &argv, const CmdInfo *const ptr_info) {
    ...
    if (!slash::string2l(str_slave_port.data(), str_slave_port.size(), (long *) &slave_port_) || slave_port_ <= 0) {
        res_.SetRes(CmdRes::kInvalidInt);
        return;
    }
...
void TrysyncCmd::Do() {
    ...
    if (!g_pika_server->FindSlave(ip_port)) {
        ...
        if (status.ok()) {
            ...
            LOG(WARNING) << "slave offset is larger than mine, slave ip: " << ip_port << " filenum: " << filenum_ <<
            " pro_offset_: " << pro_offset_;
...
void PurgelogstoCmd::DoInitial(PikaCmdArgsType &argv, const CmdInfo *const ptr_info) {
    ...
    int64_t num = 0;
    if (!slash::string2l(str_num.data(), str_num.size(), (long *) &num) || num < 0) {
        res_.SetRes(CmdRes::kInvalidParameter);
        return;
    }
    num_ = num;
...
void SelectCmd::DoInitial(PikaCmdArgsType &argv, const CmdInfo *const ptr_info) {
    ...
    int64_t db_id;
    if (!slash::string2l(argv[1].data(), argv[1].size(), (long *) &db_id)) {
        res_.SetRes(CmdRes::kInvalidInt);
    }
}
...
void InfoCmd::InfoServer(std::string &info) {
    ...
    (current_time_s / (24 * 3600) - g_pika_server->start_time_s() / (24 * 3600) + 1) << "\r\n";
...
void InfoCmd::InfoReplication(std::string &info) {
    ...
    switch (host_role) {
        case PIKA_ROLE_SLAVE :
            ...
            (g_pika_server->repl_state() == PIKA_REPL_CONNECTED ? "up" : "down") << "\r\n";
...
void DbsizeCmd::Do() {
    ...
    int32_t dbsize = (int32_t) (key_nums_v[0] + key_nums_v[1] + key_nums_v[2] + key_nums_v[3] + key_nums_v[4]);
    res_.AppendInteger(dbsize);
}
